...
//     sendMessage(currentUserId, receiverId, chatId ,newMessage);
//     setNewMessage("");
//   };
...
const Chat = ({ receiverId, projectId }: ChatProps) => {
  const { socket, messages, fetchMessages, sendMessage, loading } = useChat();
  const [newMessage, setNewMessage] = useState<string>("");
  const [currentUserId, setCurrentUserId] = useState<string | null>(null);
  ...
  const handleSendMessage = () => {
    if (!currentUserId || !newMessage.trim()) return;
    sendMessage(projectId, currentUserId, receiverId, newMessage); // Fixed function call
    setNewMessage("");
  };
